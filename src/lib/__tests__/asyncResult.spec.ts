...
describe('Result.Async is a lawful Functor, Monad, and Catamorphism — either is the endo fold, value + Result.either the exit', () => {
  const eq = async (
  ...
  test('the exit computation: value hands you the settled Result, either folds it to a plain value', async () => {
    const label = (value: number): string => `hit ${value}`;
    const blame = (reason: string): string => `miss ${reason}`;
    expect((await asyncSuccess<number, string>(3).value).either(label, blame)).toEqual('hit 3');
    expect((await asyncFailure<string, number>('e').value).either(label, blame)).toEqual('miss e');
  });

  test('the exit reflection: folding with the result constructors recovers the settled result', async () => {
    const reflect = async (m: Result.Async<number, string>): Promise<void> => {
      const settled = await m.value;
      expect(settled.either(success, failure).inspect()).toEqual(settled.inspect());
    };
    await reflect(asyncSuccess(3));
    await reflect(asyncFailure('e'));
  });

  test('the exit fusion: a plain function after the fold distributes into both branches', async () => {
    const label = (value: number): string => `hit ${value}`;
    const blame = (reason: string): string => `miss ${reason}`;
    const shout = (folded: string): string => folded.toUpperCase();
    const fused = async (m: Result.Async<number, string>): Promise<void> => {
      const settled = await m.value;
      expect(shout(settled.either(label, blame))).toEqual(
        settled.either(value => shout(label(value)), reason => shout(blame(reason)))
      );
    };
    await fused(asyncSuccess(3));
    await fused(asyncFailure('e'));
  ...
  test('the exit is a pull: cancel suppresses pushes, value still hands over the Result', async () => {
    const chain = asyncSuccess<number, string>(3);
    chain.cancel();
    expect((await chain.value).either(value => `hit ${value}`, reason => `miss ${reason}`)).toEqual('hit 3');
  });
});
